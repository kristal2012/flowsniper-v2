...
export default async function handler(req, res) {
    ...
    // Bybit
    try {
        const response = await fetchWithTimeout(`https://api.bybit.com/v5/market/tickers?category=linear&symbol=${s}`, 4000);
        ...
        console.log("Bybit fetch failed in proxy:", e.name === 'AbortError' ? 'Timeout' : e.message);
    }

    // Binance
    if (!result) {
        try {
            const response = await fetchWithTimeout(`https://api.binance.com/api/v3/ticker/price?symbol=${s}`, 4000);
            const data = await response.json();
            if (data.price) {
            ...
            console.log("Binance fetch failed in proxy:", e.name === 'AbortError' ? 'Timeout' : e.message);
        }
    }
    ...
    if (!result) {
        try {
            ...
            if (coinId) {
                const response = await fetchWithTimeout(`https://api.coingecko.com/api/v3/simple/price?ids=${coinId}&vs_currencies=usd`, 5000);
                const data = await response.json();
                const price = data[coinId]?.usd;
                if (price > 0) result = { price, source: 'coingecko' };
            }
        } catch (e) {
            console.log("CoinGecko fetch failed in proxy:", e.name === 'AbortError' ? 'Timeout' : e.message);
        }
    }
