          ...
          for (const t of volatileTokens) {
            try {
              ...
              if (Number(bal) > 0.000001) {
                const res = await fetchCurrentPrice(t.name + 'USDT').catch(() => ({ price: 0, source: 'error' }));
                portfolioValue += Number(bal) * res.price;
              }
            } catch (e) { }
          }
        }
    ...
    <div className="min-h-screen bg-[#09090b] text-zinc-100 flex flex-col md:flex-row font-['Inter'] selection:bg-[#f01a74]/30">
      ...
      <aside className="w-72 border-r border-zinc-800/50 hidden md:flex flex-col p-6 sticky top-0 h-screen bg-[#0c0c0e]">
        <div className="flex items-center gap-3 mb-10">
          ...
          <span className="font-bold text-2xl tracking-tighter bg-gradient-to-r from-white to-zinc-500 bg-clip-text text-transparent italic">FLOWSNIPER <span className="text-[10px] text-emerald-500 non-italic border border-emerald-500/20 px-1 rounded">v4.3.1 Forced Trade</span></span>
        </div>
